...
Regularized logistic regression uses regularization (penalization) to
estimate the model parameters. Regularization penalizes high valued
regression coefficients. This reduces or shrinks the coefficients
towards zero and simplifies the model. This reduces the variance (spread
of all data points) in the model. As input features are changed. the
model's prediction changes less than it would have without
regularization. Essentially regularization helps avoid over fitting.

There are two types of penalization that this model uses:

Lasso-penalties can shrink the model terms so much that they are
absolute zero (i.e. their effect is entirely removed from the model).

ridge regression- uses a different type of penalty that is most useful
for highly correlated predictors.

<br/><br/>

 We will be using the glmnet model to implement regularization.  
 
 

```{r}

rlr_spec_untuned <- logistic_reg(penalty = tune(), mixture = tune()) %>%
  set_engine("glmnet")
...
rlr_grid<-grid_regular(extract_parameter_set_dials(rlr_spec_untuned),levels=100)
...
final_rlr_params<-select_best(rlr_tune_results, "roc_auc")
...
rlr_params_table<-ggtexttable(final_rlr_params, rows = NULL, 
                        theme = ttheme("mOrange", base_size=11)) %>% 
   tab_add_title("Final RLR Parameters",face="bold") 
...
The params have a penalty that is 76% lasso and 24% weight decay.  The total amount of these penalties is .0003
...
rlr_spec<-finalize_model(rlr_spec_untuned, final_rlr_params)
...
Margins are the (perpendicular) distances between the line and those dots
closest to the line.
...
[Support Vector Machines: A Simple
Explanation](https://www.kdnuggets.com/2016/07/support-vector-machines-simple-explanation.html)
...
svn_grid<-grid_regular(extract_parameter_set_dials(svm_spec_untuned),levels=3)
...
final_svm_params<-select_best(svm_tune_results, "roc_auc")
...
svm_params_table<-ggtexttable(final_svm_params, rows = NULL, 
                        theme = ttheme("mRed", base_size=11)) %>% 
   tab_add_title("Final RLR Parameters",face="bold") 
...
svm_spec<-finalize_model(svm_spec_untuned, final_svm_params)
...
final_boost_params<-select_best(boost_tune_results, "roc_auc")
...
boost_params_table<-ggtexttable(final_boost_params, rows = NULL, 
                        theme = ttheme("mBlue", base_size=11)) %>% 
   tab_add_title("Final RLR Parameters",face="bold")
...
xgb_spec<-finalize_model(boost_spec_untuned, final_boost_params)
...
rlr_metrics <- rlr_rs %>% 
  collect_metrics(summarise = TRUE) %>%
  mutate(model = "Regularized Logistic Regression") # add the name of the model to every row
...
svm_metrics <- svm_rs %>% 
  collect_metrics(summarise = TRUE) %>%
  mutate(model = "SVM") # add the name of the model to every row
...
roc_curve<-ggarrange(rf_roc, boost_roc, svm_roc,rlr_roc,log_roc,tree_roc,knn_roc,
                  labels=c("RF","XGBoost","SVM","RLR","Log Reg","Tree","KNN"),ncol=2, nrow=4)
...
htmap_all<-ggarrange(rf_htmap, boost_htmap,svm_htmap,rlr_htmap,log_htmap,tree_htmap,knn_htmap,
                  labels=c("RF","XGBoost","SVM","RLR","Log_reg",
                  "Tree",  "Knn"),ncol=2, nrow=4)
...
*Table 13* and *Table 14* provide a clearer picture of how the models
performed on predicting the positive class (did not enroll for spring
term)

*Table 13* provides us with the sensitivity score for all models. This
score gives us an idea about when the positive class is yes and how
often does it predict yes. The formula is TP/(TP+FN). The reguarlization
logistic regression model has the highest score of 0.66. This means for
the logistic regression model just over 4 out of every 10 students in
realty are miss-labeled as enrolling for the spring term and just under
7 are correctly labeled as not enrolling for the spring, The next
closest model is the basic logistic regression model with a score of
.65.

*Figure 14* gives us the specificity score of all the models. This score
tells us when the negative class is predicted correctly (Students
enrolled for the spring. From this figure it appears all models are good
at predicting the negative class. The SVM and Random Forest models have
scores of 0.93 and 0.91 placing them at very good at predicting the
negative class. This means that just 1 out of every 10 students in
reality are miss-labeled as not enrolling for the spring term and 9 are
correctly labeled as enrolling for the spring term.
...
Models will now be evaluated on how well they perform on the test set.
It's important to determine how well a model performs on unseen data as
it is an indicator on how well it performs on outside data. We will
observe whether a model is over fit or under fit.

<br/><br/>

Over fit results in a model's failure to generalize. Sometimes we might
find that we've trained a model on some set of data, and it appears to
work well on that data, yet when we test it on some new set of data the
performance suffers. One reason this may occur is hhe model too closely
conforms to the training set and therfore misses the more generalizable
trends.

<br/><br/>

Underfitting refers to a model that can neither model the training data
nor generalize to new data. Underfitting can be avoided by using more
...
final_metrics_rlr<-collect_metrics(ug_final_rlr)

final_metrics_rlr<-final_metrics_rlr %>% 
  select(.metric, .estimate) %>% 
   pivot_wider(names_from = .metric, values_from = c(.estimate)) 
...
final_metrics_svm<-collect_metrics(ug_final_svm)

final_metrics_svm<-final_metrics_svm %>% 
  select(.metric, .estimate) %>% 
   pivot_wider(names_from = .metric, values_from = c(.estimate)) 
...
test_metrics_combined<-rbind(final_metrics_logreg,
           final_metrics_rlr, final_metrics_svm,
...
test_comb_rocauc<-test_metrics_combined %>% 
  select(model, roc_auc) %>% 
  mutate(roc_auc=round(roc_auc,2)) %>% 
...
test_comb_accuracy<-test_metrics_combined %>%   
...
model_test_sensitivity<-test_metrics_data %>% 
  select(Sensitivity )%>% 
...
sensitivity_test_table <- ggtexttable(model_test_sensitivity, 
                        theme = ttheme("mOrange", base_size=14)) %>% 
   tab_add_title("Sensitivity Scores-Test Data",face="bold", size=13) %>% 
  tab_add_footnote("Table 17", size=9)
...
model_test_precision<-test_metrics_data %>% 
  select(Precision) %>% 
...
precision_test_table <- ggtexttable(model_test_precision, 
                        theme = ttheme("mBlue", base_size=14)) %>% 
   tab_add_title("Precision Scores-Test Data",face="bold", size=13) %>% 
  tab_add_footnote("Table 13", size=9)
...
specificity_test_table <- ggtexttable(model_test_specitivity, 
                        theme = ttheme("mGreen", base_size=14)) %>% 
   tab_add_title("Specificity Scores-Test Data",face="bold", size=13) %>% 
  tab_add_footnote("Table 18", size=9)
...
htmap_test_all<-ggarrange(log_final_htmap,rlr_final_htmap,  svm_final_htmap,xgb_final_htmap,rf_final_htmap,tree_final_htmap,knn_final_htmap,
                  labels=c("Log_reg", "RLR","SVM","XGBoost","RF",
                  "Tree",  "Knn"),ncol=2, nrow=4)
...
logistic regression and regularizion logistic regression models have the highest score at 0.84 followed closely
by the SVM, XGboost, and Random Forest models.  These scores are very close to those
from the training set with the difference being the  logistic and regularization models moving to the top. With these results we can conclude that both models fit well to
unseen data. The other two models have slightly lower scores than the
training set.
...
final_reg_feat<-ug_final_rlr %>% 
  pull(.workflow) %>% 
  pluck(1) %>% 
  tidy(exponentiate=TRUE) %>% 
  arrange(estimate) 

#final_reg_feat$p.value<-round(final_reg_feat$p.value, 3)

#final_reg_feat$estimate<-round(final_reg_feat$estimate, 3)
...
reg_feat_table<- ggtexttable(reg_table, rows = NULL, 
                        theme = ttheme("mBlue", base_size=8)) %>% 
   tab_add_title("Regularization Logistic Regression Features",face="bold", size=9) %>% 
  tab_add_footnote("Table 20", size=7)
...
reg_log_features<-ug_final_rlr %>% 
  pluck(".workflow", 1) %>%   
  extract_fit_parsnip() %>% 
  vip(geom="col", aesthetics=list(fill="midnightblue", alpha = 0.8))+
  scale_y_continuous(expand=c(0,0))+
  ggtitle("Regularized Logistic Regression Feature Importance")
...
*Table 18* presents the term estimates for the Regularization logistic
regression model. Compared to the term estimates of the logistic
regression model from *Table 17* we can see the effects of penalization.
The terms race_White, age, enrolled_three.quarter.time, and degr_cde_BS
were reduced to zero thus having no affect on the model. Positive Term
estimates were marginally reduced between 5% and 75 for xfer_hrs_earned,
pct_enrolled_cr_earned. Filed_fafsa term estimate decreased by 27%. The
greatest decrease for a positive term estimate which decreased by half
(83%). The negative term estimates gender_M and race_African.American
decreased in influence by 29% and 6% respectively. Two negative term
estimates increased model influence. enrolled_half.time increased by 23%
and enrollled_less.than.half.time increases by 13%.

The regularized results show us how terms for the the model were
selected by either setting them to zero or reducing them closer to zero.
This is in contrast to the basic logistic regression model which uses
p-value to determine significant terms.

*Figure 27* plots the regularized term estimates as absolute values.
From this we can conclude enrolled.less.than.half.time has the greatest
affect on the model. The next two closest terms are trm_gpa and
enrolled.half.time at more than half the value.
...
models, feature importance does not indicate if the effects on the event
prediction is positive or negative only that the feature had a strong
effect on the prediction.

The features trm gpa and pct_enrolled_credits_earned have the greatest
importance for the decision tree model on predicting spring enrollment.
...
the impact is not as large. Moreover, xfer_hrs_earned feature is not far
behind in importance.

The XGBoost model has one feature, trm_gpa, that is dominate in
importance on predicting spring enrollment.

Overall, all three three models have term_gpa as one of the most
important features.
...
Based on the performance evaluation the logistic regression,
regularization logistic regression, SVM, xgBoost, and random forest
models all had scores that are considered good. Yet the good scores do
not necessarily mean that these models are useful for the purpose of our
analysis. Remember that the event of interest was if a student did not
return for the spring term. Yes, both the logistic regression and
regularization logistic regression models performed the best at
predicting the event of interest, each was only 70% accurate at
predicting a student not returning. Improving model performance may
require more data and finding more features for use in the fitting of
the model.
...
The models did provide insights on the first time undergraduate
population. The significant and important features gathered from each
model provides us with enough data to develop action plans focusing on
at risk students. Some recommendations are having instructors and
academic advisers work closely on monitoring at risk students at check
points during the term. Personal meetings with financial planning
advisers must be scheduled within the first week of a students
acceptance.
